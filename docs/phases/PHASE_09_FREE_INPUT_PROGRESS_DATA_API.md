...
> [!IMPORTANT]
> この文書は2026-08の方向転換前に完了した旧製品設計の実装履歴であり、現在の製品仕様・完成条件ではありません。自由入力を含む本文は履歴として保持しており、新MVPの採用仕様ではありません。現役仕様は[マスター設計書](../../マスター設計書.md)と[仕様書体系](../specs/00_README.md)を参照してください。
...
2. `/api/free-assess` を実装
   - `application/json`で自由文、Azure結果、ロケール、端末計測値を受け取る。音声は受け取らない。
   - fields:
     - `text`
     - `audio`
     - `timezone`
