...
> [!IMPORTANT]
> この文書は2026-08の方向転換前に完了した旧製品設計の実装履歴であり、現在の製品仕様・完成条件ではありません。旧称、旧課題数、旧機能は履歴として保持しています。現役仕様は[マスター設計書](../../マスター設計書.md)と[仕様書体系](../specs/00_README.md)を参照してください。
...
- 初期検証対象とする代表混同ペア10〜15件の直し方ページ作成。
...
5. 優先直し方ページを作成
   - 優先10〜15件を作成する。
   - 優先対象はL/R、TH、母音長、V/B、`theta` と `s` / `t` の混同、語末子音を中心にする。
   - 例:
     - `r_to_l`
     - `l_to_r`
     - `theta_to_s`
     ...
     - `final_d_missing`
     - `final_s_missing`
   - 直し方本文は1〜2行、最大3行に収める。初回リリースでは日本語で作成する。
